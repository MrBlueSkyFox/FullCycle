...
from pose_estimator2d.keypoints import (
    BODY_PARTS_KPT_IDS, BODY_PARTS_PAF_IDS
)
from pose_estimator2d.one_euro_filter import (
    OneEuroFilter
)
    ...
    def draw(self, img):
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1:
                ...
                cv2.circle(img, (int(x_a), int(y_a)), 9, Pose.circle_color, -1)
            kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
            global_kpt_b_id = self.keypoints[kpt_b_id, 0]
            if global_kpt_b_id != -1:
                x_b, y_b = self.keypoints[kpt_b_id]
                cv2.circle(img, (int(x_b), int(y_b)), 9, Pose.circle_color, -1)
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), Pose.line_color, 6)
